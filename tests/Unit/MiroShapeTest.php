...
test('set font family style', function() {
    $data = (new MiroShape(ShapeType::Square, 0, 'test'))->style(fn(MiroShapeStyle $style) => $style->fontFamily('Arial'));
    expect(json_decode($data->toArray()['widgetData']['json']['style'], true)['ffn'])->toBe('Arial');
});

test('set text color style', function() {
    $data = (new MiroShape(ShapeType::Square, 0, 'test'))->style(fn(MiroShapeStyle $style) => $style->textColor(16711680)->textColor('#FF0000'));
    expect(json_decode($data->toArray()['widgetData']['json']['style'], true)['tc'])->toBe(16711680);
});

test('set text align style', function() {
    $data = (new MiroShape(ShapeType::Square, 0, 'test'))->style(fn(MiroShapeStyle $style) => $style->textAlign(TextAlign::Left, VerticalTextAlign::Top));
...
test('set font size style', function() {
    $data = (new MiroShape(ShapeType::Square, 0, 'test'))->style(fn(MiroShapeStyle $style) => $style->fontSize(11));
    expect(json_decode($data->toArray()['widgetData']['json']['style'], true)['fs'])->toBe(11);
});

test('set bold style', function() {
    $data = (new MiroShape(ShapeType::Square, 0, 'test'))->style(fn(MiroShapeStyle $style) => $style->bold());
    expect(json_decode($data->toArray()['widgetData']['json']['style'], true)['b'])->toBe(1);
});

test('set italic style', function() {
    $data = (new MiroShape(ShapeType::Square, 0, 'test'))->style(fn(MiroShapeStyle $style) => $style->italic());
    expect(json_decode($data->toArray()['widgetData']['json']['style'], true)['i'])->toBe(1);
});

test('set underline style', function() {
    $data = (new MiroShape(ShapeType::Square, 0, 'test'))->style(fn(MiroShapeStyle $style) => $style->underline());
    expect(json_decode($data->toArray()['widgetData']['json']['style'], true)['u'])->toBe(1);
});

test('set strikethrough style', function() {
    $data = (new MiroShape(ShapeType::Square, 0, 'test'))->style(fn(MiroShapeStyle $style) => $style->strikethrough());
